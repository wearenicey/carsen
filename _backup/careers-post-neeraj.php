<?php
                ...
                if( $error == UPLOAD_ERR_OK && is_uploaded_file( $tmp ) ){
                    ...
                    if( $uploadOk ){
							if (move_uploaded_file( $tmp, $target_file )) {
								$mail->addAttachment($target_file, $name);
							} else {
    ...
    if( empty( $errors ) ){
        ...
        $from = 'info@carsen.se';
        $sendTo = 'careers@carsen.se';
        $subject = 'New message from Careers form';

        $mail->AddAddress( $sendTo,'neerajfeb1988@gmail.com' );
        $mail->Subject = $subject;
/*         $mail->SetFrom( $from ); */
        $mail->AltBody = 'To view the message, please use an HTML compatible email viewer!';

		$mail->From = $from;
		$mail->FromName = 'Carsen: Careers';
	...
	$mail->isHTML(true);
    $html = "<h2>You have a new message from your contact form</h2><hr>";
    $html.= "<table>";

    foreach ($_POST as $key => $value) {
        // If the field exists in the $fields array, include it in the email
        if (isset($fields[$key])) {
            $html.= "<tr><th>$fields[$key]</th><td>$value</td></tr>";
        }
    }
